...
On September 10, 2026, the project owner approved a bounded extension of that policy for the remaining website edge variance. Original component and website comparisons now use the same shared guard. Regional onboarding comparisons retain their exact policy. No reference images were changed for this approval.

The shared comparator remains exact by default. Opted-in comparisons permit a difference only when dimensions match, both pixel alphas are fully opaque, and each RGB channel differs by at most **2 out of 255**. The sample must lie between darker and lighter opaque neighbors in its immediate 3x3 neighborhood independently in both images; their summed RGB brightness must span more than six levels. Adjacent blended samples may vary together, as they did under the previous one-level policy. This conservative edge heuristic does not prove the cause of a difference and cannot distinguish every tiny local gradient-color change from rasterization variance. It never admits larger RGB changes, transparency changes, flat fills, local extrema, or size changes.

Raw differing pixels and tolerated edge pixels are counted separately and attached with the untouched actual and expected images whenever they differ. Every pixel outside the allowance fails. Playwright retains `threshold: 0`; only when the raw guard finds no rejected pixels does its matcher receive the independently verified edge count as `maxDiffPixels`. Genuine failures use `maxDiffPixels: 0` for normal diff diagnostics and still face the raw guard. There are no filename exceptions, coordinate masks, fixed pixel budgets, image rewrites, or retries based on reference agreement. Unit tests cover the recorded two-pixel neighborhood and reject larger color changes, alpha changes, flat fills, gradient tint shifts, glyph changes, pixel movement, size changes, and mixed accepted/rejected differences.
