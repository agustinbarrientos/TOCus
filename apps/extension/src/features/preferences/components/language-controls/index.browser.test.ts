...
import { BrowserLanguageLabels } from '../../types/__fixtures__/browser-labels';
import { test } from '../../../settings/utils/browser-test-harness';
import { BrowserLanguageOption } from './types';

test.describe( 'language controls', () => {
	test( 'aligns the field label with its input and preserves readable helper spacing', async ( { open } ) => {
		const page = await open( SettingsDestination.LANGUAGE );
		const selector = page.getByRole( 'combobox', { name: 'TOCus language', exact: true } );
	...
	test( 'uses the available narrow form width for Save and Discard', async ( { open } ) => {
		const page = await open( SettingsDestination.LANGUAGE );
	...
	test( 'returns keyboard focus to the native language field after Save and Discard', async ( { open } ) => {
		const page = await open( SettingsDestination.LANGUAGE );
		const selector = page.getByRole( 'combobox', { name: 'TOCus language', exact: true } );
	...
	test( 'guards navigation while explicit malformed-preference recovery is pending', async ( { open, setting } ) => {
		test.setTimeout( 20000 );
		const page = await open( SettingsDestination.LANGUAGE );
		await page.getByRole( 'combobox', { name: 'TOCus language', exact: true } ).waitFor();
		await page.evaluate( () => {
			window.settingsTest.malformPreferences();
		} );
		await setting( page, 'holdPreferenceWrites', true );
		await page.getByRole( 'button', { name: 'Restore personalization defaults', exact: true } ).click();
		expect( await page.getByRole( 'button', { name: 'Restore personalization defaults', exact: true } ).isDisabled() ).toBe( true );
	...
	test( 'renders every exact locale and persists browser-following as null with explicit Save', async ( { open, setting } ) => {
		test.setTimeout( 20000 );
		const page = await open( SettingsDestination.LANGUAGE );
		const selector = page.getByRole( 'combobox', { name: 'TOCus language', exact: true } );
		expect( await page.getByRole( 'option' ).count() ).toBe( 11 );
		for ( const language of Object.values( Language ) ) {
			expect( await page.getByRole( 'option', { name: BrowserLanguageLabels[ language ], exact: true } ).count() ).toBe( 1 );
		}
		await selector.selectOption( Language.JAPANESE );
		expect( await page.evaluate( () => window.settingsTest.controls.writes ) ).toBe( 0 );
