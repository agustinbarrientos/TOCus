...
test.describe( 'React pause', () => {
	...
	test( 'retains recovery announcements, focus and discrete reduced-motion progress', async ( { page } ) => {
		await page.goto( origin );
		await page.locator( 'html[data-ready]' ).waitFor();
		await page.evaluate( async () => {
	...
	test( 'repairs native closure and guarded removal while preserving the controlled screen', async ( { page } ) => {
	...
	test( 'keeps canvas artwork live, previews contained and detached clocks stopped', async ( { page } ) => {
		await page.goto( origin );
		await page.locator( 'html[data-ready]' ).waitFor();
	...
	test( 'keeps long localized recovery labels reachable in a short narrow viewport', async ( { page } ) => {
	...
	test( 'retains six-palette contrast, live appearance and owned styles under strict CSP', async ( { page } ) => {
		await page.goto( `${ origin }?injected&csp` );
		await page.locator( 'html[data-ready]' ).waitFor();
	...
	test( 'retains control sizing after responsive and stylesheet-driven host font changes', async ( { page } ) => {
	...
	for ( const csp of [ false, true ] ) {
		test( `isolates native dialog, packaged styles and real keyboard behavior with CSP ${ String( csp ) }`, async ( { page } ) => {
			await page.setViewportSize( { width: 800, height: 600 } );
