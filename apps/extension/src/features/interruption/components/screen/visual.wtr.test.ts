...
describe( 'tocus-f-interruption-screen visual', () => {
	...
	it( 'matches Quiet pause in the Purple dark appearance', async () => {
		document.documentElement.setAttribute( 'data-tocus-palette', 'purple' );
		document.documentElement.setAttribute( 'data-tocus-theme', 'dark' );
		...
		assert.isTrue( element.isConnected );
		await visualDiff( element, 'interruption-screen-quiet-purple-dark' );
	} );
	...
	it( 'matches the unavailable recovery state in the dark appearance', async () => {
		document.documentElement.setAttribute( 'data-tocus-palette', 'brown' );
		document.documentElement.setAttribute( 'data-tocus-theme', 'dark' );
		await emulateMedia( { colorScheme: 'dark', forcedColors: 'none', reducedMotion: 'reduce' } );
		const element = await fixture<ComponentInterruptionScreen>( html`
		...
		assert.isTrue( element.isConnected );
		await visualDiff( element, 'interruption-screen-unavailable-dark' );
	} );
} );
