...
export function createTimingCopy( i18n: I18n ): Readonly<TimingScreenCopy> {
	...
	return Object.freeze( {
		...
		maximumWaitHelp: i18n._( msg`The wait won't grow beyond this duration.` ),
		...
		malformedDataDescription: i18n._( msg`Your local timing data isn't valid, so it wasn't replaced.` ),
		loadErrorTitle: i18n._( msg`Timing settings couldn't load` ),
		loadErrorDescription: i18n._( msg`TOCus couldn't load local timing settings. Nothing was changed.` ),
		retry: i18n._( msg`Try again` ),
		save: i18n._( msg`Save` ),
		discard: i18n._( msg`Discard` ),
		saving: i18n._( msg`Saving...` ),
		saveError: i18n._( msg`Timing settings couldn't be saved. Your choices are still here.` ),
		...
		invalidTimingConfigurationError: i18n._(
			msg`These timing choices aren't valid. Review them before saving.`,
		),
		formatSecondsOption,
		formatMinutesOption,
		formatSummary,
