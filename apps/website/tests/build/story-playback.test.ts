...
for ( const engine of [ 'chromium', 'firefox', 'webkit' ] as const ) {
	...
	engineTest( `${ engine }: walkthrough plays on entry, pauses offscreen, and never opens YouTube before Continue`, async ( { page } ) => {
		...
		// Exercise consecutive frames once, then jump between observable playback checkpoints.
		await page.clock.runFor( 200 );
		const progress = story.getByRole( 'progressbar', { name: 'How TOCus works' } );
		...
		const offscreenCountdown = await page.locator( '.product-demo-countdown:visible' ).textContent();
		await page.clock.fastForward( 20_000 );
		await expect( page.locator( '.product-demo-countdown:visible' ) ).toHaveText( offscreenCountdown ?? '' );
		await page.locator( '.product-demo-browser' ).scrollIntoViewIfNeeded();
		await expect( story ).toHaveAttribute( 'data-playing', 'true' );
		// Resume establishes a fresh frame timestamp before elapsed time can advance.
